...
The Mean Reciprocal Rank, which is a metric for evaluating ranked retrieved elements. Each prediction has a rank between 1 and N and the lower the rank the higher the score given to this prediction as a match by the model.  If the ground truth image is present in the retrieved list of size N then <img src="https://latex.codecogs.com/png.latex?\dpi{50}&space;\LARGE&space;\rho" title="\LARGE \rho" /> corresponds to it's rank. Otherwise, the internal term of the sum is set to 0 (the rank is inf).
